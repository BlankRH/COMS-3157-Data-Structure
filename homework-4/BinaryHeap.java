...
public class BinaryHeap<T extends Comparable<? super T>> {
    ...
    public void insert(T x) throws IndexOutOfBoundsException {
		...
		int hole, oldObjectIndex;
		
		if(itemToArrayIndex.containsKey(x)) {
			oldObjectIndex = itemToArrayIndex.get(x);
			array[oldObjectIndex] = x;
			hole = oldObjectIndex;
		}
		else
			hole = ++currentSize;
        ...
        while (hole>1 && x.compareTo(array [hole /2]) <0 ) {
			...
			itemToArrayIndex.put(array[hole], hole);
...
/*11*/  array[ hole ] = tmp;
    ...
    public static void main( String [ ] args ) {
        ...
        h.insert(new Process("p1",50));
		 System.out.println(h.printArray());   
		h.insert(new Process("p1",5));
        System.out.println(h.printArray());         

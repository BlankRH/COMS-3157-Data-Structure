...
public class LinkedList<T> implements Iterable<T> {
  ...
  /********* ADD YOUR SOLUTIONS HERE *****************/
    public void reverse() {
        ...
        // f(5*l+7)
        // change the iterator that it begins from the endmark, get node.prev each time and stops at beginmark
      Node<T> p = beginMarker.next;
			while(p.next != null) {
				Node<T> tempPrev = p.prev;
				Node<T> tempNext = p.next;
				p.prev = tempNext;
				p.next = tempPrev;
				p = tempNext;
			}
			Node<T> tempFirstNode = beginMarker.next;
			Node<T> tempLastNode = endMarker.prev;
			tempFirstNode.next = endMarker;
			tempLastNode.prev = beginMarker;
			beginMarker.next = tempLastNode;
			endMarker.prev = tempFirstNode;
    }
